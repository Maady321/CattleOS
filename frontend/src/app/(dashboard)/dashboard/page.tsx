...
export default function OverviewPage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <section className="bg-gradient-to-br from-emerald-600 to-emerald-900 p-8 rounded-[40px] shadow-2xl shadow-emerald-500/20 relative overflow-hidden group">
        <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:scale-125 transition-transform duration-700">
          <Milk size={120} strokeWidth={1} />
        </div>
        
        <div className="relative z-10">
          <h3 className="text-emerald-200 text-sm font-black uppercase tracking-widest mb-2">Morning Production</h3>
          <div className="flex items-baseline gap-2 mb-6">
            <span className="text-6xl font-black text-white">{totalYieldToday.toFixed(1)}</span>
            <span className="text-2xl font-bold text-emerald-300">Liters</span>
          </div>
...
function StatusCard({ label, value, subText, icon, color }: any) {
  return (
    <div className="bg-surface border border-white/5 p-6 rounded-[32px] hover:border-white/10 transition-all group">
      ...
      <p className="text-[10px] font-medium text-slate-600 mt-1">{subText}</p>
    </div>
  );
}
